    ...
    <div>
        <div>
            <el-row>
                ...
                <el-col :span="3">
                    <el-button type="primary" @click="everyStepPerOrder">逐步获取结果</el-button>
                </el-col>
                <el-col :span="3">
                    <el-button type="primary" @click="oncePerOder">直接获取结果</el-button>
                </el-col>
                <el-col :span="3">
                    <div>前序遍历演示</div>
                </el-col>
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             * @param rightSon
             */
            drawPerOrder(data,parent, leftSon, rightSon) {
                this.initRect();
                this.perOrderRes();
            ...
            oncePerOder(){
                let data = this.data;
                let dataIndex = 0;
                this.perOrder(data,dataIndex);
            },

            /**
             * 递归遍历
             */
            perOrder(data,dataIndex){
                if(this.traverRes.length===data.length){
                    this.graph.clearCanvas(ctx)
                    this.drawPerOrder(data,-1,[],[]);
                }
                if(dataIndex<data.length){
                    ...
                    this.perOrder(data,leftIndex);
                    //后
                    let rightIndex = dataIndex*2+2;
                    this.perOrder(data,rightIndex)
                }
            },
            ...
            everyStepPerOrder() {
                ...
                if (this.traverRes.length >= this.data.length) {
                    //进行最后一次渲染
                    this.graph.clearCanvas(ctx);
                    this.drawPerOrder(this.data, this.parentIndex,[],[]);
                    console.log('结束了')
                    return;
                }
                //清空画布
                this.graph.clearCanvas(ctx);
                //画二叉树
                this.drawPerOrder(this.data,  this.parentIndex, leftSon, rightSon);
                ...
                // if(this.parentIndex === this.traverRes.find(item => item === this.parentIndex && !this.goleft)){
                //     // this.parentIndex = Math.floor((this.parentIndex-1)/2);
                // }
                // else{
                //     //获取根节点
                //     this.traverRes.push(this.data[this.parentIndex]);
                ...
                //     return;
                // }
                // if(!this.goleft && this.parentIndex === this.traverRes.find(item => item === this.parentIndex)){
                //
                // }
                // if(!this.goleft){
                //     this.parentIndex = this.parentIndex*2+2;
                //     this.goleft =true;
                //     return;
                // }
                // if(leftSonIndex*2+1<this.data.length){
                //     //中
                //     this.traverRes.push(this.data[this.parentIndex]);
                //     this.parentIndex = leftSonIndex;
                //     this.lay++;
                //     return;
                //     console.log('111');
                // }
                ...
                //     return;
                // }
                // else if(this.parentIndex+1<this.data.length){
                //     //中
                //     this.traverRes.push(this.data[this.parentIndex]);
                //     if(leftSonIndex < this.data.length){
                //         let leftValue = this.data[leftSonIndex]
